...
def import_from_linear(api_key: str):  # noqa: C901
    ...
    viewer_data = linear_graphql_request(
        api_key,
        """
        query {
            viewer {
    ...
    print("\nFetching teams...")
    teams_data = linear_graphql_request(
        api_key,
        """
        query {
            teams {
    ...
    # Get all labels
    print("\nFetching labels...")
    labels_data = linear_graphql_request(
        api_key,
        """
        query {
            issueLabels {
    ...
    for label in labels:
        ...
        try:
            Label.get_or_create(name=label_name, defaults={"color": label["color"]})
    ...
    for team in teams:
        ...
        try:
            project, created = Project.get_or_create(
                id=team["key"],
                defaults={"name": team["name"], "icon": "ph ph-folder", "color": "blue"},
            )
            if created:
        ...
        while has_next_page:
            ...
            issues_data = linear_graphql_request(
                api_key,
                """
                ...
                try:
                    ...
                    if created:
                            ...
                            if label_name:
                                try:
                                    TicketLabelJoin.get_or_create(
                                        ticket=ticket_id, label=label_name
                                    )
                                except Exception:
                                    pass
                        ...
                        if issue.get("assignee"):
                            ...
                            try:
                                user = User.get(
                                    User.username == assignee_name.lower().replace(" ", "")
                                )
                                UserTicketJoin.get_or_create(user=user.username, ticket=ticket_id)
...
def main():
    parser = argparse.ArgumentParser(
        prog="broke", description="Broke CLI - Issue tracking and error monitoring"
    )
    ...
    linear_parser.add_argument(
        "api_key", help="Linear API key (get from https://linear.app/settings/account/security)"
    )
