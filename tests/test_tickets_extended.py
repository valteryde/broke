...
def _(c=auth_client, ticket=sample_ticket):
    ...
    response = c.put(f'/api/tickets/{ticket.id}', 
...
def _(c=auth_client, ticket=sample_ticket):
    ...
    response = c.patch(f'/api/tickets/{ticket.id}',
...
def _(c=auth_client, ticket=sample_ticket, label=sample_label):
    ...
    existing = TicketLabelJoin.select().where(
        (TicketLabelJoin.ticket == ticket.id) & 
        (TicketLabelJoin.label == label.name)
    ).first()
    if not existing:
...
def _(c=auth_client, project=sample_project):
    ...
    response = c.post('/api/tickets', data=json.dumps({
        'title': '',
        'description': 'Test',
