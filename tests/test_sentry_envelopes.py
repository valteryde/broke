...
def dsn_token(app=app, part=sentry_project_part):
    ...
    token = DSNToken.create(
        token=f"test-dsn-{int(time.time() * 1000000)}",
...
def _(c=client, part=sentry_project_part, token=dsn_token):
    ...
    error = ErrorGroup.select().where(ErrorGroup.project_part == part.id).first()
...
def _(c=client, part=sentry_project_part, token=dsn_token):
    ...
    response = c.post(
        ...
        data=compressed,
        headers={
            'X-Sentry-Auth': f'Sentry sentry_key={token.token}',
...
def _(c=client, part=sentry_project_part, token=dsn_token):
    ...
    event2 = {
        ...
        "exception": {
            "values": [{
                ...
                "stacktrace": {
                    "frames": [
                        {"filename": "app.py", "function": "main", "lineno": 42}  # Same location
                    ]
                }
            }]
        }
    ...
    error_groups = ErrorGroup.select().where(ErrorGroup.project_part == part.id)
    assert len(list(error_groups)) == 1
    
    # Verify there are 2 occurrences
    error_group = error_groups.first()
    assert error_group.count == 2
