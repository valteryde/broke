...
def test_project(fake:faker.Faker=fake):
    project = Project.create(id=str(fake.uuid4()), name=fake.word(), icon='ph ph-folder', color='blue')
    yield project
...
def test_ticket(fake:faker.Faker=fake, test_project:Project=test_project):
    ticket = Ticket.create(id=str(fake.uuid4()), title=fake.sentence(), description=fake.text(), project=test_project.id, status='todo', priority='medium')
    yield ticket
